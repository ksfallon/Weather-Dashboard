...
function handleCitySearchSubmit(event) {
  ...
  var searchInputVal = cityInput.value.trim();
...
function getWeather(search) {
  ...
  var geocodeQueryUrl = `https://api.openweathermap.org/geo/1.0/direct?q=${search},&limit=5&appid=af8f9e641174c07751bae2f5bbbc3fb5&units=imperial`;
  ...
  .then(function (response) {
    ...
    console.log('response 2:', response[0].name);
    ...
    console.log('response 3:', response[0]);
    ...
    console.log('response 4:', cityToSearch.lat, "and", cityToSearch.lon );
    $.ajax({ url: oneCallWeatherApi })
  
    .then(function (response) {
      
      var iconCodeToday = response.current.weather[0].icon
      console.log('response 5:', response.current.weather[0].icon)
  ...
  function displayLocalStorage() {
    ...
    if (displayCitySearch) {
      ...
      $('#previous-search').prepend(`<button class="pastSearch w3-btn bbtn-light btn-block mt-3 text-dark" id = "${displayCitySearch[i]}"  onclick="clickRecentCity(event)">${displayCitySearch[i]}</button>`)
    }
  }
}

const clickRecentCity = (event) => {
// get the city name by targeting the text content of that event.
  let cityName = event.target.textContent;
  console.log("city-name: ", cityName);
//know its sent to getWeather because console.loging has shown it is - not sure why its not displaying on the screen yet!
  getWeather(cityName);
}
